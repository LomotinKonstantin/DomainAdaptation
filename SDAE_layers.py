...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 2:
        ...
        total_epochs = ae_config.getint(ae_section, "epochs")
        ...
        train_model(model,
                    ...
                    log_fname=None,
                    memlog_fname=None,
                    epochs=min(step, total_epochs - epoch),
                    ae=True, noise_decorator=gaussian_noise)
        epoch += step
        if epoch >= total_epochs:
            layers = eval(ae_config.get(ae_section, "layers"))
            for _ in range(int(len(layers) / 2) - 1):
                model.layers.pop()
        ...
        epoch = 0
    Popen("python3 SDAE_layers.py {}".format(str(epoch)), shell=True)
